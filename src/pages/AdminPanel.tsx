...
const AdminPanel = () => {
  ...
  // New state for overpowered admin tools
  const [massCoinsAmount, setMassCoinsAmount] = useState(1000);
  const [setAllBalanceAmount, setSetAllBalanceAmount] = useState(10000);
  const [megaKeyAmount, setMegaKeyAmount] = useState(100000);
  const [megaKeyQuantity, setMegaKeyQuantity] = useState(10);
  ...
  const giveCoinsToAll = async () => {
    if (!massCoinsAmount || massCoinsAmount <= 0) {
    ...
    try {
      const { data, error } = await supabase.rpc('admin_give_coins_to_all', {
        coin_amount: massCoinsAmount
      });

      if (error) throw error;
      
      toast.success(`Gave ${massCoinsAmount} coins to ${data} users!`);
      fetchUserProfiles();
    } catch (error) {
  ...
  const setAllBalances = async () => {
    if (!setAllBalanceAmount || setAllBalanceAmount < 0) {
    ...
    try {
      const { data, error } = await supabase.rpc('admin_set_all_balances', {
        target_balance: setAllBalanceAmount
      });

      if (error) throw error;
      
      toast.success(`Set balance to ${setAllBalanceAmount} for ${data} users!`);
      fetchUserProfiles();
    } catch (error) {
  ...
  const clearAllGameHistory = async () => {
    setLoading(true);
    try {
  ...
  const createMegaKeys = async () => {
    ...
    for (let i = 0; i < megaKeyQuantity; i++) {
      keysToCreate.push({
        code: 'MEGA-' + generateRandomString(16),
        amount: megaKeyAmount
      });
    }
    ...
    if (error) {
      ...
      toast.success(`Created ${megaKeyQuantity} MEGA KEY${megaKeyQuantity > 1 ? 'S' : ''} worth ${megaKeyAmount} coins each!`);
      fetchCoinKeys();
    }
    setLoading(false);
  };
        ...
        <Tabs defaultValue="keys" className="w-full">
          ...
          <TabsContent value="tools" className="space-y-4 md:space-y-6">
            <Card className="bg-gray-800 border-gray-700">
              ...
              <CardContent className="space-y-4">
                {/* OVERPOWERED ADMIN SECTION */}
                <div className="bg-red-900/20 p-4 rounded-lg border border-red-600/50">
                  <div className="flex items-center gap-2 mb-4">
                    <Shield className="w-5 h-5 text-red-400" />
                    <h4 className="font-mono text-red-400 text-lg">⚡ OVERPOWERED ADMIN CONTROLS ⚡</h4>
                  </div>
                  
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    {/* Mass Coin Distribution */}
                    <div className="p-3 bg-gray-750 rounded-lg border border-yellow-600/50">
                      <h5 className="font-mono text-yellow-400 text-sm mb-2">💰 Mass Coin Distribution</h5>
                      <Input
                        type="number"
                        value={massCoinsAmount}
                        onChange={(e) => setMassCoinsAmount(parseInt(e.target.value) || 1000)}
                        className="bg-gray-700 border-gray-600 text-white font-mono text-xs mb-2"
                        placeholder="Coins to give all users"
                        min={1}
                      />
                      <Button
                        onClick={giveCoinsToAll}
                        disabled={loading}
                        className="w-full bg-yellow-600 hover:bg-yellow-700 text-black font-mono text-xs"
                      >
                        Give {massCoinsAmount} Coins to ALL Users
                      </Button>
                    </div>

                    {/* Set All Balances */}
                    <div className="p-3 bg-gray-750 rounded-lg border border-green-600/50">
                      <h5 className="font-mono text-green-400 text-sm mb-2">🎯 Set All Balances</h5>
                      <Input
                        type="number"
                        value={setAllBalanceAmount}
                        onChange={(e) => setSetAllBalanceAmount(parseInt(e.target.value) || 10000)}
                        className="bg-gray-700 border-gray-600 text-white font-mono text-xs mb-2"
                        placeholder="New balance for all users"
                        min={0}
                      />
                      <Button
                        onClick={setAllBalances}
                        disabled={loading}
                        className="w-full bg-green-600 hover:bg-green-700 text-white font-mono text-xs"
                      >
                        Set ALL Users to {setAllBalanceAmount} Coins
                      </Button>
                    </div>

                    {/* Mega Key Creator */}
                    <div className="p-3 bg-gray-750 rounded-lg border border-purple-600/50">
                      <h5 className="font-mono text-purple-400 text-sm mb-2">🔑 MEGA Key Creator</h5>
                      <div className="grid grid-cols-2 gap-2 mb-2">
                        <Input
                          type="number"
                          value={megaKeyAmount}
                          onChange={(e) => setMegaKeyAmount(parseInt(e.target.value) || 100000)}
                          className="bg-gray-700 border-gray-600 text-white font-mono text-xs"
                          placeholder="Coins per key"
                          min={1}
                        />
                        <Input
                          type="number"
                          value={megaKeyQuantity}
                          onChange={(e) => setMegaKeyQuantity(parseInt(e.target.value) || 10)}
                          className="bg-gray-700 border-gray-600 text-white font-mono text-xs"
                      ...
                      <Button
                        ...
                        disabled={loading}
                        className="w-full bg-purple-600 hover:bg-purple-700 text-white font-mono text-xs"
                      >
                        Create {megaKeyQuantity} MEGA Keys ({megaKeyAmount} each)
                      </Button>
                    </div>

                    {/* Reset All Stats */}
                    <div className="p-3 bg-gray-750 rounded-lg border border-orange-600/50">
                      <h5 className="font-mono text-orange-400 text-sm mb-2">📊 Nuclear Options</h5>
